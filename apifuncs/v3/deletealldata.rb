...
post '/v3/deletealldata.?:format?' do
  ...
  begin
    ...
    if authResult[:authenticated]
      ...
      @db.query("DELETE FROM twitter_users WHERE sw_uid='#{sw_uid}'")
      @db.query("DELETE FROM facebook_users WHERE sw_uid='#{sw_uid}'")
      @db.query("DELETE FROM sw_users WHERE sw_uid='#{sw_uid}'")
