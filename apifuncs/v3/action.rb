...
# and a verb identifying the action: 'retweet', 'unretweet', 'favorite', or
# 'like'.
      ...
      if params.has_key?('service') && params.has_key?('uid') && params.has_key?('postid') && params.has_key?('action')
        ...
        if params['service'] == 'twitter'
          ...
          if twitter_users.num_rows == 1
            ...
            status 403
            returnHash[:success] = false
            returnHash[:error] = "A post was requested via Twitter user ID @#{params['uid']}, but that account is not known to SuccessWhale."
          end
          ...
          if facebook_users.num_rows == 1
            ...
            if user['sw_uid'].to_i == sw_uid
              ...
              status 403
              returnHash[:success] = false
              returnHash[:error] = "A post was requested via a Facebook account with uid #{params['uid']}, but the authenticated user does not have the right to use this account."
            end
          else
            status 403
            returnHash[:success] = false
            returnHash[:error] = "A post was requested via a Facebook account with uid #{params['uid']}, but that account is not known to SuccessWhale."
          end
